 ...
 */
/*		1.0.8	 21/10/19   Ajout Compensation alti gps et température
 * et enable bip */
/*                      au démarrage */
 ...
 */
/*											Ajout
 * saveConfigurationVario / loadConfigurationVario		 */
/* 		1.1.0	 13/11/19		Modification soundSettingRead /
 * soundSettingWrite        */
/*                      Ajout lecture et sauvegarde fichier wifi.cfg
 */
/*										  Ajout gestion
 * automatique des version fichier params.jso */
/*										  Modif
 * ALTERNATE_DATA_DURATION - MULTIDISPLAY_DURATION	   */
  ...
  if (myFile)
  {
    while (myFile.available())
    {
      ...
      if (character == ']')
      {
        ...
        if (applySetting(settingName, settingValue))
          *ModifiedValue = true;
        // Reset Strings
        settingName = "";
        settingValue = "";
...
boolean VarioSettings::applySetting(String settingName, String settingValue)
{
  ...
  {
    /* The near climbing alarm : signal that you enter or exit the near climbing
     * zone */
  ...
  else if (settingName == "VARIOMETER_SENT_LXNAV_SENTENCE")
  {
    ...
    VARIOMETER_SENT_LXNAV_SENTENCE =
        settingValue.toInt(); // toBoolean(settingValue);
  }
...
void VarioSettings::loadConfigurationVario(char *filename)
{
  ...
  //2*JSON_OBJECT_SIZE(5) + JSON_OBJECT_SIZE(11) + JSON_OBJECT_SIZE(12) + 790;
  //	const size_t capacity = JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(3) +
  //3*JSON_OBJECT_SIZE(5) + JSON_OBJECT_SIZE(12) + JSON_OBJECT_SIZE(13) + 1090;
  //  const size_t capacity = JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(3) +
  //  3*JSON_OBJECT_SIZE(5) + JSON_OBJECT_SIZE(14) + JSON_OBJECT_SIZE(17) +
  //  1090; DynamicJsonDocument doc(capacity);
  ...
  if (tmpValue == 1)
    VARIOMETER_ENABLE_BT = true;
  else
    VARIOMETER_ENABLE_BT = false;
#ifdef SDCARD_DEBUG
  ...
  if (tmpValue == 1)
    ALARM_VARIOBEGIN = true;
  else
    ALARM_VARIOBEGIN = false;
#ifdef SDCARD_DEBUG
  ...
  {
    String General_GLIDER_GLIDER_NAME =
        General_GLIDER["GLIDER_NAME1"]; // "MAC-PARA Muse 3"
  ...
  {
    String General_GLIDER_GLIDER_NAME2 =
        General_GLIDER["GLIDER_NAME2"]; // "MAC-PARA Muse 3"
  ...
  {
    String General_GLIDER_GLIDER_NAME3 =
        General_GLIDER["GLIDER_NAME3"]; // "MAC-PARA Muse 3"
  ...
  {
    String General_GLIDER_GLIDER_NAME4 =
        General_GLIDER["GLIDER_NAME4"]; // "MAC-PARA Muse 3"
  ...
  if (tmpValue == 1)
    VARIOMETER_ENABLE_NEAR_CLIMBING_ALARM = true;
  else
    VARIOMETER_ENABLE_NEAR_CLIMBING_ALARM = false;
  ...
  if (tmpValue == 1)
    VARIOMETER_ENABLE_NEAR_CLIMBING_BEEP = true;
  else
    VARIOMETER_ENABLE_NEAR_CLIMBING_BEEP = false;
  ...
  if (tmpValue == 1)
    VARIOMETER_DISPLAY_INTEGRATED_CLIMB_RATE = true;
  else
    VARIOMETER_DISPLAY_INTEGRATED_CLIMB_RATE = false;
  ...
  if (tmpValue == 1)
    VARIOMETER_ENABLE_AGL = true;
  else
    VARIOMETER_ENABLE_AGL = false;
  ...
  if (tmpValue == 1)
    VARIOMETER_INTEGRATED_CLIMB_RATE = true;
  else
    VARIOMETER_INTEGRATED_CLIMB_RATE = false;
  ...
  if (tmpValue == 1)
    BLUETOOTH_SEND_CALIBRATED_ALTITUDE = true;
  else
    BLUETOOTH_SEND_CALIBRATED_ALTITUDE = false;
  ...
  if (tmpValue == 1)
    ...
    VARIOMETER_RECORD_WHEN_FLIGHT_START = false;
...
void VarioSettings::saveConfigurationVario(char *filename)
{
  ...
  if (ALARM_SDCARD)
    ...
    Systeme["ALARM_SDCARD"] = 0;

  if (ALARM_GPSFIX)
    Systeme["BEEP_GPSFIX"] = 1;
  else
    Systeme["BEEP_GPSFIX"] = 0;

  if (ALARM_FLYBEGIN)
    Systeme["BEEP_FLYBEGIN"] = 1;
  else
    Systeme["BEEP_FLYBEGIN"] = 0;

  if (ALARM_VARIOBEGIN)
    Systeme["BEEP_VARIOBEGIN"] = 1;
  else
  ...
  if (VARIOMETER_ENABLE_NEAR_CLIMBING_ALARM)
    Vario["ENABLE_NEAR_CLIMBING_ALARM"] = 1;
  else
    Vario["ENABLE_NEAR_CLIMBING_ALARM"] = 0;

  if (VARIOMETER_ENABLE_NEAR_CLIMBING_BEEP)
    Vario["ENABLE_NEAR_CLIMBING_BEEP"] = 1;
  else
    Vario["ENABLE_NEAR_CLIMBING_BEEP"] = 0;

  if (VARIOMETER_DISPLAY_INTEGRATED_CLIMB_RATE)
    Vario["DISPLAY_INTEGRATED_CLIMB_RATE"] = 1;
  else
  ...
  if (VARIOMETER_ENABLE_AGL)
    Vario["VARIOMETER_ENABLE_AGL"] = 1;
  else
    Vario["VARIOMETER_ENABLE_AGL"] = 0;

  /*
  if (VARIOMETER_SENT_LXNAV_SENTENCE) Systeme["SENT_LXNAV_SENTENCE"] = 1;
  else
  Systeme["SENT_LXNAV_SENTENCE"] = 0;
*/
  Vario["VARIOMETER_SENT_LXNAV_SENTENCE"] = VARIOMETER_SENT_LXNAV_SENTENCE;

  Vario["ACCELERATION_MEASURE_STANDARD_DEVIATION"] =
      ACCELERATION_MEASURE_STANDARD_DEVIATION;

  if (VARIOMETER_INTEGRATED_CLIMB_RATE)
    Vario["VARIOMETER_INTEGRATED_CLIMB_RATE"] = 1;
  else
    Vario["VARIOMETER_INTEGRATED_CLIMB_RATE"] = 0;

  Vario["SETTINGS_VARIO_PERIOD_COUNT"] = SETTINGS_VARIO_PERIOD_COUNT;

  if (BLUETOOTH_SEND_CALIBRATED_ALTITUDE)
    Vario["BLUETOOTH_SEND_CALIBRATED_ALTITUDE"] = 1;
  else
    Vario["BLUETOOTH_SEND_CALIBRATED_ALTITUDE"] = 0;
  ...
  Flight_start["FLIGHT_START_VARIO_LOW_THRESHOLD"] =
      FLIGHT_START_VARIO_LOW_THRESHOLD;
  /*  SerialPort.print("FLIGHT_START_VARIO_LOW_THRESHOLD : ");
  SerialPort.println(FLIGHT_START_VARIO_LOW_THRESHOLD);*/

  Flight_start["FLIGHT_START_VARIO_HIGH_THRESHOLD"] =
  ...
  if (VARIOMETER_RECORD_WHEN_FLIGHT_START)
    Flight_start["RECORD_WHEN_FLIGHT_START"] = 1;
  else
    Flight_start["RECORD_WHEN_FLIGHT_START"] = 0;
  /*  SerialPort.print("RECORD_WHEN_FLIGHT_START : ");
  SerialPort.println(VARIOMETER_RECORD_WHEN_FLIGHT_START);*/
...
//**********************************************************
void VarioSettings::setVersion(uint8_t version, uint8_t subVersion,
                               uint8_t betaVersion)
{
  //**********************************************************
  GnuvarioVersion = String(version) + "." + String(subVersion);
  if (betaVersion > 0)
    GnuvarioVersion += "b" + String(betaVersion);
}
...
/*
{
    ...
    "Systeme": {
        ...
        "BEEP_FLYBEGIN": 1,
                                ...
                                "URL_UPDATE":
"http://gnuvario-e.yj.fr/webupdate/checkversion", "LANGUAGE": 0, "REF_VOLTAGE":
2280
    },
    "General": {
...
const size_t capacity = JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(3) +
3*JSON_OBJECT_SIZE(5) + JSON_OBJECT_SIZE(11) + JSON_OBJECT_SIZE(12) + 940;
DynamicJsonDocument doc(capacity);

const char* json =
"{\"GnuvarioE\":{\"version\":\"1.0\"},\"Systeme\":{\"BT_ENABLE\":0,\"NO_RECORD\":0,\"ALARM_SDCARD\":1,\"BEEP_GPSFIX\":1,\"BEEP_FLYBEGIN\":1,\"BEEP_VARIOBEGIN\":0,\"COMPENSATION_TEMP\":-6.1,\"COMPENSATION_GPSALTI\":-70,\"SLEEP_TIMEOUT_MINUTES\":20,\"SLEEP_THRESHOLD_CPS\":50,\"ALTERNATE_DATA_DURATION\":2000},\"General\":{\"PILOT_NAME\":\"MagaliXXXXXXXXXXXXXX\",\"GLIDER\":{\"GLIDER_SELECT\":0,\"GLIDER_NAME\":\"MAC-PARA
Muse 3XXXXX\",\"GLIDER_NAME2\":\"MAC-PARA Muse
3XXXXX\",\"GLIDER_NAME3\":\"MAC-PARA Muse 3XXXXX\",\"GLIDER_NAME4\":\"MAC-PARA
Muse
3XXXXX\"},\"TIME_ZONE\":1},\"Vario\":{\"SINKING_THRESHOLD\":-2.1,\"CLIMBING_THRESHOLD\":0.2,\"NEAR_CLIMBING_SENSITIVITY\":0.5,\"ENABLE_NEAR_CLIMBING_ALARM\":0,\"ENABLE_NEAR_CLIMBING_BEEP\":0,\"DISPLAY_INTEGRATED_CLIMB_RATE\":0,\"CLIMB_PERIOD_COUNT\":10,\"SETTINGS_GLIDE_RATIO_PERIOD_COUNT\":20,\"RATIO_MAX_VALUE\":30.1,\"RATIO_MIN_SPEED\":10.1,\"RATIO_CLIMB_RATE\":2,\"SENT_LXNAV_SENTENCE\":1},\"Flight
start\":{\"FLIGHT_START_MIN_TIMESTAMP\":15000,\"FLIGHT_START_VARIO_LOW_THRESHOLD\":-0.5,\"FLIGHT_START_VARIO_HIGH_THRESHOLD\":0.5,\"FLIGHT_START_MIN_SPEED\":8.1,\"RECORD_WHEN_FLIGHT_START\":1}}";
...
int Systeme_ALTERNATE_DATA_DURATION = Systeme["ALTERNATE_DATA_DURATION"]; //
2000
...
const char* General_GLIDER_GLIDER_NAME = General_GLIDER["GLIDER_NAME"]; //
"MAC-PARA Muse 3" const char* General_GLIDER_GLIDER_NAME2 =
General_GLIDER["GLIDER_NAME2"]; // "" const char* General_GLIDER_GLIDER_NAME3 =
General_GLIDER["GLIDER_NAME3"]; // "" const char* General_GLIDER_GLIDER_NAME4 =
General_GLIDER["GLIDER_NAME4"]; // ""
...
float Vario_NEAR_CLIMBING_SENSITIVITY = Vario["NEAR_CLIMBING_SENSITIVITY"]; //
0.5 int Vario_ENABLE_NEAR_CLIMBING_ALARM = Vario["ENABLE_NEAR_CLIMBING_ALARM"];
// 0 int Vario_ENABLE_NEAR_CLIMBING_BEEP = Vario["ENABLE_NEAR_CLIMBING_BEEP"];
// 0 int Vario_DISPLAY_INTEGRATED_CLIMB_RATE =
Vario["DISPLAY_INTEGRATED_CLIMB_RATE"]; // 0 int Vario_CLIMB_PERIOD_COUNT =
Vario["CLIMB_PERIOD_COUNT"]; // 10 int Vario_SETTINGS_GLIDE_RATIO_PERIOD_COUNT =
Vario["SETTINGS_GLIDE_RATIO_PERIOD_COUNT"]; // 20 float Vario_RATIO_MAX_VALUE =
Vario["RATIO_MAX_VALUE"]; // 30.1 float Vario_RATIO_MIN_SPEED =
Vario["RATIO_MIN_SPEED"]; // 10.1 int Vario_RATIO_CLIMB_RATE =
Vario["RATIO_CLIMB_RATE"]; // 2 int Vario_SENT_LXNAV_SENTENCE =
Vario["SENT_LXNAV_SENTENCE"]; // 1

JsonObject Flight_start = doc["Flight start"];
int Flight_start_FLIGHT_START_MIN_TIMESTAMP =
Flight_start["FLIGHT_START_MIN_TIMESTAMP"]; // 15000 float
Flight_start_FLIGHT_START_VARIO_LOW_THRESHOLD =
Flight_start["FLIGHT_START_VARIO_LOW_THRESHOLD"]; // -0.5 float
Flight_start_FLIGHT_START_VARIO_HIGH_THRESHOLD =
Flight_start["FLIGHT_START_VARIO_HIGH_THRESHOLD"]; // 0.5 float
Flight_start_FLIGHT_START_MIN_SPEED = Flight_start["FLIGHT_START_MIN_SPEED"];
// 8.1 int Flight_start_RECORD_WHEN_FLIGHT_START =
Flight_start["RECORD_WHEN_FLIGHT_START"]; // 1

SERIALIZING

const size_t capacity = JSON_OBJECT_SIZE(1) + JSON_OBJECT_SIZE(3) +
3*JSON_OBJECT_SIZE(5) + JSON_OBJECT_SIZE(11) + JSON_OBJECT_SIZE(12);
DynamicJsonDocument doc(capacity);
