...
/*  int i,j,plus=0;
  ...
  if (w < box_x) 	{
	  ...
	  Zwidth   = w-box_x;//Zwidth   = w-box_x+2;
	}
	Zheight  = 24;//Zheight  = 24+6; = hauteur des caracteres
}
...
//****************************************************************************************************************************
  ...
  /* build digit */
  /***************/
//***********************************************************************************Taille des Digits dans le fenetres varioscreenobject
#ifdef SCREEN_DEBUG
  ...
  if (leftAlign) {
    ...
    display.print(digitCharacters);
		...
		if ((anchorX-w+2) < 0) w = anchorX - 2;//if ((anchorX-w+2) < 0) w = anchorX - 2;
																								//COULEUR DES BLOCS EN NOIR
#ifdef SCREEN_DEBUG
    ...
    display.setCursor(anchorX-w1-2, anchorY-1); //display.setCursor(anchorX-w1-2, anchorY-1);
    display.print(digitCharacters);
	}
...
*/	
 ...
 /* uint16_t baseVoltage = base + inc;
  ...
  if (Voltage >= 2160)
    display.drawInvertedBitmap(273, 8, bat4icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else if ((Voltage < 2160) && (Voltage >= 2100))
    display.drawInvertedBitmap(273, 8, bat3icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else if ((Voltage < 2100) && (Voltage >= 2000))
    display.drawInvertedBitmap(273, 8, bat2icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else if ((Voltage < 2000) && (Voltage >= 1750))
    display.drawInvertedBitmap(273, 8, bat1icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else {
...
//	display.drawRect(posX, posY, 32, 32, GxEPD_BLACK);
  ...
  if (pVoltage >= 75)
    display.drawInvertedBitmap(277, 8, bat4icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else if (pVoltage >= 50)
    display.drawInvertedBitmap(posX, posY, bat3icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else if (pVoltage >= 25)
    display.drawInvertedBitmap(posX, posY, bat2icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else if (pVoltage >= 10)
    display.drawInvertedBitmap(posX, posY, bat1icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
  else
    display.drawInvertedBitmap(posX, posY, bat0icons, 17, 8, GxEPD_BLACK);   //GxEPD_BLACK);
...
0xff, 0xef, 0xff, 0xff, 0xcf, 0xff, 0xff, 0x8f, 0xff, 0xff, 0x0f, 0xff, 0xfe, 0x0f, 0xff, 0xc0, 
0x0d, 0xfd, 0xc0, 0x0e, 0xfb, 0xc0, 0x0f, 0x77, 0xc0, 0x0f, 0xaf, 0xc0, 0x0f, 0xdf, 0xc0, 0x0f, 
0xaf, 0xc0, 0x0f, 0x77, 0xc0, 0x0e, 0xfb, 0xc0, 0x0d, 0xfd, 0xfe, 0x0f, 0xff, 0xff, 0x0f, 0xff, 
0xff, 0x8f, 0xff, 0xff, 0xcf, 0xff, 0xff, 0xef, 0xff
};

const unsigned char volume1icons[] = { 
 // 'basic1-093_volume_low-32'
0xff, 0xef, 0x80, 0xff, 0xcf, 0x80, 0xff, 0x8f, 0x80, 0xff, 0x0f, 0x80, 0xfe, 0x0f, 0x80, 0xc0, 
0x0f, 0x80, 0xc0, 0x0e, 0x80, 0xc0, 0x0e, 0x00, 0xc0, 0x0f, 0x00, 0xc0, 0x0f, 0x00, 0xc0, 0x0f, 
0x00, 0xc0, 0x0e, 0x00, 0xc0, 0x0e, 0x80, 0xc0, 0x0f, 0x80, 0xfe, 0x0f, 0x80, 0xff, 0x0f, 0x80, 
0xff, 0x8f, 0x80, 0xff, 0xcf, 0x80, 0xff, 0xef, 0x80
};

const unsigned char volume2icons[] = { 
0xff, 0xef, 0xf0, 0xff, 0xcf, 0xf0, 0xff, 0x8f, 0xf0, 0xff, 0x0f, 0xf0, 0xfe, 0x0f, 0xb0, 0xc0, 
0x0f, 0x90, 0xc0, 0x0e, 0xd0, 0xc0, 0x0e, 0x40, 0xc0, 0x0f, 0x60, 0xc0, 0x0f, 0x60, 0xc0, 0x0f, 
0x60, 0xc0, 0x0e, 0x40, 0xc0, 0x0e, 0xd0, 0xc0, 0x0f, 0x90, 0xfe, 0x0f, 0xb0, 0xff, 0x0f, 0xf0, 
0xff, 0x8f, 0xf0, 0xff, 0xcf, 0xf0, 0xff, 0xef, 0xf0
};

const unsigned char volume3icons[] = { 
 // 'basic1-095_volume_loud-32'
0xff, 0xef, 0xff, 0xff, 0xcf, 0xff, 0xff, 0x8f, 0xf7, 0xff, 0x0f, 0xf7, 0xfe, 0x0f, 0xb3, 0xc0, 
0x0f, 0x9b, 0xc0, 0x0e, 0xdb, 0xc0, 0x0e, 0x49, 0xc0, 0x0f, 0x6d, 0xc0, 0x0f, 0x6d, 0xc0, 0x0f, 
0x6d, 0xc0, 0x0e, 0x49, 0xc0, 0x0e, 0xdb, 0xc0, 0x0f, 0x9b, 0xfe, 0x0f, 0xb3, 0xff, 0x0f, 0xf7, 
...
const unsigned char saticons[] = { 
 // 'signal-24x20'
0xff, 0xff, 0xf0, 0xff, 0xff, 0xf0, 0xff, 0xff, 0xf0, 0xff, 0xff, 0xf0, 0xff, 0xfe, 0x10, 0xff, 
0xfe, 0x10, 0xff, 0xfe, 0x10, 0xff, 0xc2, 0x10, 0xff, 0xc2, 0x10, 0xff, 0xc2, 0x10, 0xf8, 0x42, 
0x10, 0xf8, 0x42, 0x10, 0xf8, 0x42, 0x10, 0x08, 0x42, 0x10, 0x08, 0x42, 0x10, 0x08, 0x42, 0x10, 
...
//****************************************************************************************************************************
  ...
  display.drawInvertedBitmap(posX, posY, saticons, 24, 20, GxEPD_BLACK);   //GxEPD_BLACK);
  ...
  display.fillRect(posX+(satelliteBar*6), posY, 24-(satelliteBar*6), 20, GxEPD_WHITE);
...
//****************************************************************************************************************************
  ...
  if( recordState==STATE_RECORD) {
	  ...
	  if( displayRecord)
        display.drawInvertedBitmap(posX, posY,recicons,  24, 17, GxEPD_BLACK);   //GxEPD_BLACK);
	  else
	    display.fillRect(posX, posY, 24, 24, GxEPD_WHITE);
  } else if  (recordState==STATE_GPSFIX) {
      display.drawInvertedBitmap(posX, posY,pauseicons,  24, 24, GxEPD_BLACK);   //GxEPD_BLACK);
  } else if  (recordState==STATE_NORECORD) {
      display.drawInvertedBitmap(posX, posY,norecordicons,  24, 24, GxEPD_BLACK);   //GxEPD_BLACK);
  } else if (displayRecord) {
      display.drawInvertedBitmap(posX, posY,fix1icons,  16, 16, GxEPD_BLACK);   //GxEPD_BLACK);
  } else  {
      display.drawInvertedBitmap(posX, posY,fix2icons,  16, 16, GxEPD_BLACK);   //GxEPD_BLACK);
  }
}
...
const unsigned char fixgpsicons[] = { 
 // 'location-marker-24'
0xfe, 0x1f, 0xf8, 0x07, 0xf0, 0xc3, 0xf1, 0xe3, 0xe1, 0xe1, 0xe1, 0xe1, 0xe0, 0xc1, 0xe0, 0x01, 
0xf0, 0x03, 0xf0, 0x03, 0xf0, 0x03, 0xf8, 0x07, 0xf8, 0x07, 0xfc, 0x0f, 0xfe, 0x1f, 0xff, 0x3f
};
...
// display.drawRect(posX, posY, 24, 24, GxEPD_BLACK);
 ...
 if (FixGPS == true) display.drawInvertedBitmap(posX, posY, fixgpsicons, 16, 16, GxEPD_BLACK);   //GxEPD_BLACK);
...
// display.drawBitmap(msicons, posX, posY, 48, 48, GxEPD_WHITE,false);   //GxEPD_BLACK);
 if (InfoValue == INFO_USB) display.drawInvertedBitmap(posX, posY, usbicons, 24, 24, GxEPD_BLACK);   //GxEPD_BLACK);
...
const unsigned char bticons [] PROGMEM = {
	// 'BT icone 24x24, 19x24px
	0xff, 0x81, 0xe0, 0xff, 0x00, 0x60, 0xfe, 0x00, 0x20, 0xfc, 0x00, 0x00, 0xf8, 0x08, 0x00, 0xf8, 
	0x0c, 0x00, 0xf8, 0x0a, 0x00, 0xf8, 0x09, 0x00, 0xf8, 0x89, 0x00, 0xf8, 0xca, 0x00, 0xf8, 0x6c, 
	0x00, 0xf8, 0x38, 0x00, 0xf8, 0x38, 0x00, 0xf8, 0x6c, 0x00, 0xf8, 0xca, 0x00, 0xf8, 0x89, 0x00, 
	0xf8, 0x09, 0x00, 0xf8, 0x0a, 0x00, 0xf8, 0x0c, 0x00, 0xf8, 0x08, 0x00, 0xfc, 0x00, 0x20, 0xfe, 
	0x00, 0x60, 0xff, 0x00, 0xe0, 0xff, 0x81, 0xe0
};
